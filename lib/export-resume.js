...
const getThemePkg = (theme) => {
  if (!path.isAbsolute(theme)) {
    if (theme[0] === '.') {
      ...
      theme = path.join(theme, 'index.js');
    }
  }
  try {
...
const createPdf = (resumeJson, fileName, theme, format, callback) => {
  (async () => {
    ...
    const mediaType = (themePkg.pdfRenderOptions && themePkg.pdfRenderOptions.mediaType) ||
        'print';
    ...
    console.log('Generating PDF with options', pdfOpts, 'media type:', mediaType, 'viewport:', themePkg.pdfViewport);
